...
Name Pankaj Yadav AND NETID(S) HERE
16 March, 2023
"""
...
def is_angle(ang):
    """
    Returns: True ang is number between 0 to 180; False otherwise

    Parameter ang: the value to check
    Precondition: None (ang can be any value)
    """
    return (type(ang) == int or type(ang) == float) and 0 < ang < 180
...
def draw_spiral(w, side, ang, n, sp):
    ...
    assert is_valid_speed(sp), report_error('sp is not a valid speed',sp)
    assert is_angle(ang), report_error('invalid ang', ang)
...
def draw_spiral_helper(t, side, ang, n, sp):
    ...
    red_seq = [] # taking accumulator to store the number of which red color draw
    for i in range(n + 1):
        red_seq.append(3*(i + 1) - 2)
        if i % 3 == 0:
            t.color = 'blue'
        elif i in red_seq:
            t.color = 'red'
...
def multi_polygons(w, side, k, n, sp):
    ...
    assert is_valid_length(side), report_error('side is not a valid length',side)
    assert is_valid_speed(sp), report_error('sp is not a valid speed',sp)
    assert is_number(k) and k >= 1, report_error('k is not valid number of polygons', k)
    assert is_number(n) and n >= 3, report_error('n is not valid number os side',n)
...
def vicsek(w, size, d, sp):
    ...
    # ARE THESE ALL OF THE PRECONDITIONS?
    assert is_window(w), report_error('w is not a valid window',w)
    assert is_valid_length(size), report_error('size is not a valid length',side)
...
def vicsek_helper(p, x, y, size, d):
    ...
    # ARE THESE ALL OF THE PRECONDITIONS?
    assert is_valid_penmode(p), report_error('Invalid pen mode', p)
    assert is_number(x), report_error('x-coordinate is not valid',x)
    assert is_number(y), report_error('y-coordinate is not valid',y)
    assert is_valid_depth(d), report_error('d is not a valid depth',d)
...
def branches(w, hght, ang, d, sp):
    ...
    # ARE THESE ALL OF THE PRECONDITIONS?
    assert is_window(w), report_error('w is not a valid window', w)
    assert is_number(hght), report_error('Invalide side length', hght)
    assert is_angle(ang), report_error('invalid ang', ang)
    assert is_valid_depth(d), report_error('d is not a valid depth', d)
    assert is_valid_speed(sp), report_error('sp is not a valid speed', sp)
...
def branches_helper(t, hght, ang, d):
    ...
    The tree is draw with the current turtle color, and assuming that 
